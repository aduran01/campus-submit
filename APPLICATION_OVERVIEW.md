...
- **Git history** — the plaintext seed passwords that had been committed across prior commits (three distinct values across the admin and student accounts, plus two early SHA-256 digests of those same values from a pre-backend prototype phase) were scrubbed from every commit's blob content via `git filter-repo --replace-text`, and the repo's history was rewritten and force-pushed — all 11 commits got new hashes. See the conversation/commit log for the exact before/after commit-hash record, since that's a one-time operational step rather than something this living doc needs to keep re-describing.
